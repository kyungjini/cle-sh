...
def main():
    ...
    for model in model_dirs:
        ...
        path_result = os.path.join(model_path, "clesh_results")
        ...
        # Generate report
        report_path = os.path.join(model_path, f"clesh_report_{model}.md")
        report = open(report_path, "w")

        report.write(f"# CLE-SH results for {LABEL} - Model: {model}\n")
        ...
        for i_rank, i_t in enumerate(rank_feat):
            ...
            read = open(
                path_result + f"/univariate_analysis/_tmp/{name_target}.txt", "r"
            )
            list_line = []
            ...
            if not list_line:
                print(f"Warning: No data found in {name_target}.txt for model {model}")
                continue
        ...
        for i_rank, (i_t, i_i) in enumerate(idx_pair):
            ...
            read = open(
                path_result + f"/interactive_analysis/_tmp/{name_target}.txt", "r"
            )
            list_line = []
