...
def feat_ttest(
    data_shap, df_importance, rank_feat, p_feature_selection=0.05, path_save=None
):
    # SHAP t-test heatmap
    plt.tight_layout()
...
def feature_selection(
    ...
    if manual_num != 0:
        ...
        text.write(
            "* caution: rank cut optimization failed, candidate_num_min selected"
        )
        text.close()
...
def discrete_target(
    ...
    if idx_interaction is not None:
        ...
        if class_interaction is not None:
            ...
            feat_target = data_feat[
                data_feat[name_interaction] == value_interaction
            ].iloc[:, idx_target]
    ...
    if idx_interaction is not None:
        ...
        statistic_target(
            ...
            p_bound=p_bound,
            bool_uni=bool_uni,
            path_save=f'{make_dir(path_save+"/univariate_analysis")}',
            name_target=name_target,
        )
...
def continuous_target(
    ...
    if idx_interaction is not None:
        ax.scatter(
            x_in, y_in, alpha=0.5, s=5, facecolors="none", edgecolors=colorset[n_c]
        )
    else:
        ...
        try:
            ...
            if func == "sigmoid":
                for method in ["trf", "dogbox", "lm"]:
                    try:
                        popt, _ = curve_fit(
                            locals()[func], x, y, p0, method=method, maxfev=2000
                        )
                ...
                for method in ["trf", "dogbox", "lm"]:
                    try:
                        popt, _ = curve_fit(
                            locals()[func], x, y, method=method, maxfev=2000
                        )
                        popt_cand.append(popt)
                        y_pred_real = locals()[func](x, *popt)
                        error_func_cand.append(rmse(y, y_pred_real))
