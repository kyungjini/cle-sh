...
def main():
    ...
    if not os.path.exists(path_load):
        raise FileNotFoundError(f"Data path does not exist: {path_load}")
    ...
    for model in model_dirs:
        ...
        data_shap_path = os.path.join(model_path, "shap.npy")

        if not os.path.exists(data_shap_path):
            print(f"!! No shap.npy found in {model_path}. Skipping...")
            continue

        data_shap = np.load(data_shap_path)
        df_importance, rank_feat = feat_importance_ranking(data_feat, data_shap)

        path_save = os.path.join(model_path, "clesh_results")
        os.makedirs(path_save, exist_ok=True)
        path_tmp = os.path.join(path_save, "_tmp")
        os.makedirs(path_tmp, exist_ok=True)
        path_univariate = os.path.join(path_save, "univariate_analysis")
        os.makedirs(path_univariate, exist_ok=True)
        path_uni_tmp = os.path.join(path_univariate, "_tmp")
        os.makedirs(path_uni_tmp, exist_ok=True)
        path_interaction = os.path.join(path_save, "interactive_analysis")
        os.makedirs(path_interaction, exist_ok=True)
        ...
        np.save(os.path.join(path_tmp, "feat_type.npy"), feat_type)
        ...
        for i_rank, i_t in enumerate(feat_tosee):
            ...
            if feat_type[i_t] != 1:
                try:
                    ...
                    print(f"Error in discrete_target for feature {i_t}: {e}")
                    continue
                ...
                plt.savefig(
                    os.path.join(path_univariate, f"{name_target}.jpg"),
                    dpi=300,
                )
                ...
                text = open(
                    os.path.join(path_uni_tmp, f"{name_target}.txt"),
                    "a",
                )
        ...
        np.save(os.path.join(path_tmp, "idx_pair.npy"), idx_pair)
                ...
                if feat_type[i_i] != 1:
                    for value_interaction in sorted(data_feat.iloc[:, i_i].unique()):
                        ...
                        plt.savefig(
                            os.path.join(
                                path_interaction,
                                "{name_target}_{name_interaction}_{value_interaction}.jpg",
                            ),
                            dpi=500,
                        )
                    ...
                    sec.set_xticks(
                        [tick[0], tick[int(len(tick) / 2)]],
                        labels=["\nupper", "\nlower"],
                    )
                    plt.tight_layout()
                    plt.savefig(
                        os.path.join(
                            path_interaction, f"{name_target}_{name_interaction}.jpg"
                        ),
                        bbox_inches="tight",
                        dpi=500,
                    )
