...
interaction_list = False  # default=False
...
for dataset in datasets:
    path_load = f"./samples_for_demo/{dataset}/data"
    path_save = make_dir(f"./samples_for_demo/{dataset}/clesh_results")

    data_feat = pd.read_csv(f"{path_load}/features.csv")
    data_feat = data_feat.drop(data_feat.columns[0], axis=1)
    data_shap = np.load(f"{path_load}/shap.npy")
    ...
    plt.savefig(f"{path_save}/shap_summary_plot.jpg", dpi=300)
    plt.clf()
    feat_type = feat_type_describe(data_feat=data_feat, path_save=path_save)
    np.save(f"{path_save}/_tmp/feat_type.npy", feat_type)
    ...
    np.save(f"{path_save}/_tmp/idx_pair.npy", idx_pair)
        ...
        if feat_type[i_t] != 1:
            ...
            if feat_type[i_i] != 1:
                for value_interaction in sorted(data_feat.iloc[:, i_i].unique()):
                    ...
                    plt.savefig(
                        f'{make_dir(path_save+"/interaction_analysis")}/{name_target}_{name_interaction}_{value_interaction}.jpg',
                        dpi=500,
                    )
                    plt.show()
